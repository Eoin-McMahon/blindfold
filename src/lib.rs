...
pub fn generate_gitignore_file(languages: Vec<&str>, file_map: HashMap<String, String>) -> String {
    ...
    for language in languages.iter() {
        if file_map.contains_key(&language.to_string()) {
            ...
            println!("Couldn't generate template for {}, did you mean {}? [y/n]", language.yellow().bold(), most_similar.bright_green().bold());
            stdin().read_line(&mut choice)
                .ok()
                .expect("Couldn't read line");

            if choice.to_lowercase() == String::from("y\n") {
                println!("Generating .gitignore for {}... 🔧", most_similar.magenta().bold());
                let ignore_body: String = get_ignore_file(&file_map, &most_similar);
                let ignore_template: String = format!("# {} gitignore generated by Blindfold\n\n{}\n\n", most_similar.to_uppercase(), ignore_body);
                gitignore.push_str(&ignore_template);
            }
        }
